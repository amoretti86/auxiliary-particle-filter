...
def integrate_gaussian(grid, weight, mean, std, f):
    ...
    value = np.dot(f(zz), weight)/np.sqrt(np.pi) # exponentiate the mean for the transformed case
    return value
...
def apf(obs, time, n_particles, n_gridpoints, A, B, Sigma, Gamma, x_0):
    ...
    dimension = 2 #B.shape[1] #obs.shape[1]
    ...
    for t in range(1, time):
        ...
        for i in range(n_particles):
            ...
            # g_int_func = make_gaussian_pdf(b*X[i,t-1], gamma)
            # g_int_func = make_poisson_pmf(obs[t])
            g_mean1 = b*obs[t,:]#np.dot(b,obs[t,:]).ravel() #or B*obs[t,:]
            g_mean = fhn(g_mean1,.01)
            g_int_func = make_mvn_pdf(g_mean,Gamma) #### PROBLEM IS HERE OR ABOVE when defining G!!
            #pdb.set_trace()
            k[i,t] = bivariate_gauss_hermite(xt, wt, fhn(X[i,t-1,:],.01), T, g_int_func, XX, YY)#np.dot(A,X[i, t-1,:]), T, g_int_func, XX, YY )
        ...
        for i in range(n_particles):
            ...
            #pdb.set_trace()
            g = make_mvn_pdf(reshaped_g_mean,Gamma)(obs[t,:])#(obs[t,:])
            q = make_mvn_pdf(X[i,t-1,:],np.identity(dimension))(X[i,t,:])#sp.stats.norm.pdf(X[i,t],X[i,t-1],1)
            f = make_mvn_pdf(fhn(X[i,t-1,:],.1),Sigma)(X[i,t,:])#np.dot(A,X[i,t-1,:]), Sigma)(X[i,t,:])
            #pdb.set_trace()
            W[i,t] = (g*f)/(k[i,t]*q)
